...
/// <summary>
/// Керує вагами рігів (aim / hands / additive) з трьох джерел:
///  - інпут/сигнал (ADS),
///  - Animator параметри (fallback),
///  - явні виклики SetReloading/SetHasWeapon.
/// Null-safe, автопризначає Animator.
/// </summary>
...
public class RigWeightBlender : MonoBehaviour
{
    ...
    void Reset() => AutoWire();
    void Awake() { AutoWire(); CacheAnimatorParams(); }

    private void AutoWire()
    {
        if (!anim) anim = GetComponent<Animator>();
        if (!aimRig) aimRig = GetComponentInChildren<Rig>(true);
        // handsRig/additiveRig — залишаємо руками через інспектор (менше шансів хибно вгадати)
    }
    ...
    /// <summary> Пряме встановлення Aim (0=hip,1=ADS) якщо не хочеш залежати від Animator/інпуту. </summary>
    public void SetAim01(float aim01)
    {
        Apply(aim01, ReadSprinting(), ReadReloading());
    }

    // === Update loop (fallback/комбайн) =======================================
    void Update()
    {
        // 1) Джерело aim01: сигнал/інпут/Animator
        float aim01 = 0f;
        if (_input != null) aim01 = _input.IsAimHeld() ? 1f : 0f;
        else if (useAnimatorAimFallback && anim && _hasAimFloatParam) aim01 = anim.GetFloat(aimFloatParam);
        // else лишається 0f
    ...
    private void Apply(float aim01, bool sprinting, bool reloading)
    {
        ...
        if (additiveRig) LerpRig(additiveRig, 1f);
    }
    ...
    private bool ReadSprinting() => anim && _hasSprintParam && anim.GetBool(paramIsSprinting);
    private bool ReadReloading()
    {
        bool v = _explicitReloading;
        if (anim && _hasReloadParam) v |= anim.GetBool(paramIsReloading);
        return v;
    }
}
