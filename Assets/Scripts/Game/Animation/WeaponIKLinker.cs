...
public class WeaponIKLinker : MonoBehaviour
{
	...
	private void Update()
	{
		if (_points == null || !handsRig)
			return;

		// 1) t беремо з Animator Aim01 (єдиний source-of-truth),
		//    fallback – інпут (якщо немає Animator або Aim01).
		float targetT = 0f;

		Animator anim = GetComponentInParent<Animator>();
		if (anim != null && anim.HasFloat("Aim01"))
		{
			targetT = anim.GetFloat("Aim01");
		}
		else if (_input != null)
		{
			targetT = _input.IsAimHeld() ? 1f : 0f;
		}

		_t = Mathf.MoveTowards(_t, targetT, aimLerpSpeed * Time.deltaTime);

		// 2) Лерпимо кожну точку між hip і ADS
